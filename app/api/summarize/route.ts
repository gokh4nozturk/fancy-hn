...
export async function POST(request: Request) {
	try {
		...
		if (!hfResponse.ok) {
			const errorData = await hfResponse
				.json()
				.catch(() => ({ error: "API yanıt hatası" }));
			...
			if (errorData.error?.includes("Rate limit")) {
				return NextResponse.json(
					{
						error:
							"Çok fazla istek yapıldı. Lütfen biraz bekleyin ve tekrar deneyin.",
					},
					{ status: 429 },
				);
			}
		...
		const summary = Array.isArray(result)
			...
			: result[0]?.summary_text || "Özet oluşturulamadı";
